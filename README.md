...
### Prueba mock de WhatsApp con webhook.site

Se probó el nodo HTTP Request que simula el envío por WhatsApp Cloud API usando **webhook.site** como endpoint mock. Esto permite validar que el workflow arma correctamente el payload dinámico sin usar credenciales reales de Meta ni enviar mensajes reales.

El payload generado incluye:

- `messaging_product`: `whatsapp`
- `to`: teléfono normalizado del lead
- `type`: `text`
- `text.body`: mensaje personalizado con nombre y `trace_id`

<p align="center">
  <img src="assets/whatsapp-mock-01.png" alt="Webhook.site mock endpoint" width="80%" />
</p>

<p align="center">
  <img src="assets/whatsapp-mock-02.png" alt="n8n WhatsApp mock workflow execution" width="90%" />
</p>

<p align="center">
  <img src="assets/whatsapp-mock-03.png" alt="Webhook.site received WhatsApp mock payload" width="90%" />
</p>

### Persistencia real en PostgreSQL Docker

También se probó persistencia real usando un contenedor **PostgreSQL Docker aislado** para este portfolio, sin tocar bases de datos de otros proyectos.

Se validaron dos tablas:

- `portfolio_leads`: guarda leads validados.
- `portfolio_incidents`: guarda incidentes clasificados por severidad y SLA.

Esto demuestra que los workflows no solo envían notificaciones, sino que también pueden dejar trazabilidad en base de datos.

<p align="center">
  <img src="assets/postgres-evidence-03.png" alt="Lead workflow with PostgreSQL persistence" width="90%" />
</p>

<p align="center">
  <img src="assets/postgres-evidence-02.png" alt="Incident workflow with PostgreSQL persistence" width="90%" />
</p>

<p align="center">
  <img src="assets/postgres-evidence-01.png" alt="Google Sheets and persistence evidence" width="90%" />
</p>
...
### WhatsApp mock test with webhook.site

The HTTP Request node that simulates WhatsApp Cloud API delivery was tested using **webhook.site** as a mock endpoint. This validates that the workflow builds the dynamic payload correctly without using real Meta credentials or sending real WhatsApp messages.

The generated payload includes:

- `messaging_product`: `whatsapp`
- `to`: normalized lead phone number
- `type`: `text`
- `text.body`: personalized message with name and `trace_id`
...
### Real persistence with PostgreSQL Docker

Real persistence was also tested using an isolated **PostgreSQL Docker** container for this portfolio, without touching databases from other projects.

Two tables were validated:

- `portfolio_leads`: stores validated leads.
- `portfolio_incidents`: stores incidents classified by severity and SLA.

This demonstrates that the workflows do more than send notifications: they also provide database traceability.
